...
function writeData(st, data) {
    ...
    return dbPromise
        ...
        var transaction = dbObject.transaction(st , 'readwrite');
...
function readAllData(st){
    return dbPromise
        .then(function (dbObject) {
            var transaction = dbObject.transaction(st,'readonly');
            var store = transaction.objectStore(st);
            return store.getAll();
        })
}
...
function readReviewsData(st,id){
    return dbPromise
        .then(function (dbObject) {
            var transaction = dbObject.transaction(st,'readonly');
            var store = transaction.objectStore(st).index('restaurant_id');
